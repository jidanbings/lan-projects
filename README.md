...
| 🔗 **持久配对** | 26 字符高熵配对码（128 位）或二维码绑定设备，密钥离网建立、服务器不可见，长期使用无需重复配对 |
| 🌍 **多语言** | 支持 30+ 种语言界面 |
...
4. 首页右上角「⚙ 设置」内可**检查更新**（GitHub 获取并安装最新版本）、查看**更新日志**、管理**已授权的权限**、查看**隐私与 SDK** 说明
...
| 连接类型 | 加密方式 | 说明 |
|----------|----------|------|
| 🔗 **已配对设备** | 128 位配对密钥派生（HKDF-SHA256）+ **ChaCha20-Poly1305 AEAD** 加密 | 密钥只在两台设备之间建立（线下扫码 / 输码），**服务器永远接触不到密钥**；同一网络抓包解不开，服务器中毒也解不开、改不动 |

页面底部显示 **🔒 端对端加密** 表示传输已加密。

> 加密使用 ChaCha20-Poly1305（纯 JS 实现，机密性 + 完整性认证一体）+ 随机 nonce，无需 HTTPS 证书即可在局域网正常工作。文件内容、文件名、大小、聊天文本全部加密，服务器只转发密文。
>
> **未配对的设备不能互传文件**——先点顶栏 🔗 配对（扫码或输入高熵配对码），配对后即可安全传输。
...
1. 点按顶部 **🔗 配对图标**
2. **发起方**扫描二维码或向对方展示 **26 字符高熵配对码**（分组显示，如 `ABCDE-FGHIJ-KLMNO-PQRST-UVWX2`）
3. **接收方**在配对弹窗中输入 26 字符配对码（可粘贴，自动忽略空格 / 连字符）
4. 配对成功后自动存储，下次无需重新配对
5. 配对后即可互传文件，传输全程 ChaCha20-Poly1305 端到端加密

> **⚠️ 历史配对失效**：新版本采用零信任密钥协商（密钥离网建立，服务器不再发放密钥），**旧版本的已配对设备需重新配对**。
...
- **权限可控**：「设置 → 已授权的权限」可逐项查看 / 开关 App 所需的可管理权限（相机 / 通知 / 附近 WiFi / 悬浮窗 / 安装未知应用），每个权限都写明用途与拒绝影响；「显示在其他应用上层」仅用于选文件时保持屏幕常亮防断连，**不代表有广告悬浮窗**
- **检查更新走加速代理**：仅当你主动点「设置 → 检查更新」时才联网，App 查询 GitHub 最新版本并下载安装包；国内直连 GitHub 不通时会**自动经第三方加速代理**（ghfast.top 等）转发下载、GitHub 直连兜底，请求不携带任何个人数据；下载的安装包在对应版本安装完成后自动清理
