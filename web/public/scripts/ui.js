class PeersUI {
    ...
    async _onKeyDown(e) {
        ...
        // close About lan-projects page on Escape
        if (e.key === "Escape") {
...
class Dialog {
    ...
    hide() {
        ...
        document.title = 'lan-projects | 跨平台局域网文件共享';
        changeFavicon("images/web.ico");
    ...
    async _displayFiles(peerId, displayName, connectionHash, files, imagesOnly, totalSize, badgeClassName) {
        ...
        if (files.length > 1) {
            ...
            try {
                ...
                filenameDownload = `lan-projects_files_${year+month+date}_${hours+minutes}.zip`;
            } catch (e) {
                console.error(e);
        ...
        this.$downloadBtn.onclick = _ => {
            if (downloadZipped) {
                ...
                tmpZipBtn.href = url;
                // Android WebView's DownloadListener does NOT receive a.download,
                // so tell the native bridge the real name BEFORE the click. The
                // bridge is only present inside the Android app (missing in a
                // desktop browser), hence the guard.
        ...
        document.title = files.length === 1
            ? `${ Localization.getTranslation("document-titles.file-received") } - lan-projects`
            : `${ Localization.getTranslation("document-titles.file-received-plural", null, {count: files.length}) } - lan-projects`;
        changeFavicon("images/web.ico");
...
class ReceiveRequestDialog extends ReceiveDialog {
    ...
    _showRequestDialog(request, peerId) {
        ...
        document.title =  `${transferRequestTitle} - lan-projects`;
        changeFavicon("images/web.ico");
...
class ReceiveTextDialog extends Dialog {
    ...
    _setDocumentTitleMessages() {
        document.title = this._receiveTextQueue.length <= 1
            ? `${ Localization.getTranslation("document-titles.message-received") } - lan-projects`
            : `${ Localization.getTranslation("document-titles.message-received-plural", null, {count: this._receiveTextQueue.length + 1}) } - lan-projects`;
    }
...
class WebShareTargetUI {
    ...
    async evaluateShareTarget(shareTargetType, title, text, url) {
        if (shareTargetType === "text") {
            ...
            let openRequest = window.indexedDB.open('lan_projects_store')
