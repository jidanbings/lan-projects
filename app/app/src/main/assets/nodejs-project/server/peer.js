...
export default class Peer {
    ...
    _setIP(request) {
        ...
        if (this.conf.debugMode) {
            ...
            console.debug("lan-projects uses:", this.ip);
        ...
        // Save real LAN IP before overwrite (for WebRTC ICE mDNS fix)
        this.lanIp = null;
    ...
    addRoomSecret(roomSecret) {
        // `in` checks array keys (indices), not values, so it never matched a
        // secret string — duplicates were added every time. Use includes().
